...
rd.build_ResNet_train_dataloader(datasets=['dhfr'], rootdir='./', savedir='./', limit_one_dataloader=100)
dataloader = np.load('./train_dataloader_0_mask8.npy', allow_pickle=True).item()
print(dataloader['dhfr'][0].shape, dataloader['dhfr'][1].shape)


rd.build_ResNet_tm_dataloader(datasets=['dhfr'], rootdir='./', savedir='./', limit_one_dataloader=100)
dataloader = np.load('./tm_dataloader_0_mask8.npy', allow_pickle=True).item()
print(dataloader['dhfr'].shape)