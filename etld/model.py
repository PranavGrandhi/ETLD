...
def Train2(model, optim_schedule, dataloader1,  dataloader2, checkpoint,
        batch_size, epochs, dataset=None, trainlog='log.csv', printepoch=False):
    '''When the source MSA and the target MSA are different, the user needs to prepare the dataloader for both MSAs.
    Dataloader1 for the source MSA, and Dataloader2 for the target MSA.
    '''
    
    if dataset is None:
        dataset = dataloader1.dataset + '-' + dataloader2.dataset
    assert dataloader1.pad_index == dataloader2.pad_index
    assert dataloader1.nseqs == dataloader2.nseqs

    base_src_tensors = dataloader1.train_tensors
    base_weights = dataloader1.weights
    seq_len = int((dataloader1.seq_len + dataloader2.seq_len) / 2)
    times_of_seq_len = dataloader1.times_of_seq_len

    base_tgt_tensors = dataloader2.train_tensors

    if times_of_seq_len <= 0:
        seq_nums_of_each_epoch = dataloader1.nseqs
    else:
        seq_nums_of_each_epoch = int(times_of_seq_len * seq_len)

    # create TM_mask, which can be None
    TM_mask = creat_TM_mask(src_seq_len=dataloader1.seq_len, tgt_seq_len=dataloader2.seq_len)
    TM_mask = torch.tensor(TM_mask).to(model.device, dtype=torch.float32)
    ...
    pad_index = dataloader1.pad_index
    best_acc = 0.
    ...
    for epoch in range(epochs):
        ...
        train_loss_sum = 0.
        train_acc_sum = 0.
        if dataloader1.nseqs > seq_nums_of_each_epoch:
            select_idx = random.sample(idx, seq_nums_of_each_epoch)
            src_tensors = base_src_tensors[select_idx]
            tgt_tensors = base_tgt_tensors[select_idx]
            weights = base_weights[select_idx]
            idx = list(np.arange(seq_nums_of_each_epoch))
        else:
            src_tensors = base_src_tensors
            tgt_tensors = base_tgt_tensors
            weights = base_weights
        np.random.shuffle(idx)
        # weights = dataloader.batch_weightCal(idx, 512)
        for train_batch, i in enumerate(range(0, src_tensors.size(0), batch_size), start=1):
            batch_index = idx[i:i + batch_size]
            train_loss, train_acc = train_step(model, optim_schedule, src_tensors[batch_index, :],
                                                   tgt_tensors[batch_index, :], weights[batch_index], TM_mask, pad_index)
            ...
            train_loss_sum += train_loss
            train_acc_sum += train_acc
        train_loss_sum /= dataloader1.Neff
        lr = optim_schedule.get_last_lr()[0]
        record = (epoch, lr, train_loss_sum / train_batch, train_acc_sum / train_batch)
        df_history.loc[epoch] = record
